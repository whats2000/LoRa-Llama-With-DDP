...
def train(
    ...
    # exp06 e06d: restricted 4-way option loss — option first-token ids (" A".." D").
    _opt_first_ids = [tokenizer.encode(f" {c}", add_special_tokens=False)[0] for c in ["A", "B", "C", "D"]]
    _opt_ids_t = torch.tensor(_opt_first_ids, device=accelerator.device)
    _tok_to_opt = {tid: i for i, tid in enumerate(_opt_first_ids)}
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch in tqdm(
            ...
            with accelerator.accumulate(model):
                # Restricted 4-way option loss: cross-entropy over {A,B,C,D} at the
                # answer position only — directly optimises the inference objective
                # (option log-likelihood) instead of full-vocab next-token CE.
                logits = model(
                ...
                L = logits.size(1)
                ans_pos = (pl - 1).clamp(min=0, max=L - 1)        # logits here predict the letter
                ans_logits = logits[ar, ans_pos, :]               # (B, V)
                opt_logits = ans_logits[:, _opt_ids_t]            # (B, 4)
                gold_tok = batch["input_ids"][ar, pl.clamp(max=L - 1)]
                gold = torch.tensor(
                    [_tok_to_opt.get(int(t), 0) for t in gold_tok.tolist()],
                    device=logits.device,
                )
                loss = F.cross_entropy(opt_logits, gold)
