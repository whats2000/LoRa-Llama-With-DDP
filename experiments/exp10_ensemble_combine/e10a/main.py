...
def main() -> None:
    ...
    combine = str(cfg["ensemble"].get("combine", "arithmetic"))
    val_list, bench_list, weights, per_member = [], [], [], {}

    for m in members:
        strat = str(m["strategy"])
        pc = _prompt_cfg_for(base_prompt_cfg, strat)
        shots = int(m.get("shots", base_prompt_cfg.get("num_few_shot_examples", 4)))
        ex = _build_few_shot_examples(train_df, shots) if strat == "few_shot" else None
        ...
        per_member[tag] = acc
        print(f"  [{tag}] single val LL-acc = {acc:.4f}  weight={float(m.get('weight', 1.0))}")
        val_list.append(vp); bench_list.append(bp); weights.append(float(m.get("weight", 1.0)))

    def _combine(prob_list):
        P = torch.stack(prob_list)                       # (M, N, 4)
        w = torch.tensor(weights).view(-1, 1, 1)
        if combine == "geometric":
            agg = (w * torch.log(P.clamp_min(1e-9))).sum(0) / w.sum()
            return torch.softmax(agg, dim=-1)
        return (w * P).sum(0) / w.sum()                  # arithmetic

    val_ens = _combine(val_list)
    bench_ens = _combine(bench_list)
    ens_val_acc = (val_ens.argmax(1) == val_gold).float().mean().item()
    print(f"\nENSEMBLE ({len(members)} members, combine={combine}) val LL-acc = {ens_val_acc:.4f}")
    ...
    summary = {"members": [f"{m['strategy']}:{m['adapter']}" for m in members],
               "combine": combine, "weights": weights,
