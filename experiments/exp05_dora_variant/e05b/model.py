...
def load_model_and_tokenizer(
    ...
    lora_config = LoraConfig(
        ...
        # exp05: config-driven LoRA-method variants.
    ...
    print(f"  use_dora={bool(lora_cfg.get('use_dora', False))} "
          f"use_rslora={bool(lora_cfg.get('use_rslora', False))} "
          f"init={lora_cfg.get('init_lora_weights', True)} "
          f"r={int(lora_cfg['r'])} alpha={int(lora_cfg['lora_alpha'])}")
